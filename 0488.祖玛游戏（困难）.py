...
class Solution:
    def findMinStep(self, board: str, hand: str) -> int:
        ...
        while queue:
            board, hand, step = queue.pop(0)
            for i, j in product(range(len(board)), range(len(hand))):
                ...
                if board[i] == hand[j]:
                    flag = True
                if i > 0 and board[i-1] == board[i] and board[i] != hand[j]:
                    flag = True

                if flag:
                    new_board = eliminate(board[:i] + hand[j] + board[i:])
                    if not new_board:
                        return step + 1
