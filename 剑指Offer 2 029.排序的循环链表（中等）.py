...
class Solution:
    def insert(self, head: 'Node', insertVal: int) -> 'Node':
        ...
        while pre:
            ...
            if pre.val <= insertVal and nxt.val > insertVal:
                pre.next = Node(insertVal, nxt)
                return head
            elif pre.val >= insertVal and nxt.val >= insertVal and pre.val > nxt.val:
                pre.next = Node(insertVal, nxt)
                return head
            elif pre.val <= insertVal and nxt.val <= insertVal and pre.val > nxt.val:
                pre.next = Node(insertVal, nxt)
                return head
            elif nxt == head and count > 0:
                pre.next = Node(insertVal, nxt)
                return head
            else:
                pre = nxt
